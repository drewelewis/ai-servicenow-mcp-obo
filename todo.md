...
- [x] Reconcile refresh-capable Copilot Studio OAuth scopes during `azd up`, verify persisted connector state without exposing its secret, and deploy revision `ca-mcp-y3p3ykro4eike--azd-1786543324`. (2026-08-12)
- [x] Accept and test both valid Entra audience forms emitted for the broker API (`client-id` and `api://client-id`) in incoming Copilot Studio tokens; deployed revision `ca-mcp-y3p3ykro4eike--azd-1786481061` is Running/Healthy. (2026-08-11)
- [x] Automate Power Platform connector callback discovery and multi-URI Entra registration in every configured `azd up`, retaining a manual callback override for headless environments; verified with revision `ca-mcp-y3p3ykro4eike--azd-1786455738`. (2026-08-11)
- [x] Retrieved both generated Copilot Studio connector callbacks, registered them on the Copilot Studio Entra client through `azd up`, and verified revision `ca-mcp-y3p3ykro4eike--azd-1786453548` and the MCP initialization endpoint. (2026-08-11)
- [x] Completed a full `azd up` with all four Entra registrations reconciled, root `.env` synchronized, infrastructure provisioned, image remotely built, and revision `ca-mcp-y3p3ykro4eike--azd-1786403010` verified Running/Healthy with an HTTP 200 JSON-RPC `/mcp` initialization response. (2026-08-10)
- [x] Consolidated ServiceNow JWT, Entra OBO, and Copilot Studio configuration into one root `.env` and removed all generated/nested env intermediaries and merge tooling. (2026-08-10)
- [x] Write all Copilot Studio manual OAuth fields generated by `azd up` to the git-ignored `.env`, including Refresh URL, with exact README field mapping and focused coverage. (2026-08-10)
- [x] Documented why azd provisions four Entra registrations and clarified which apps serve Copilot Studio production, local testing, and optional direct OBO. (2026-08-10)
- [x] Integrated all Entra app provisioning into the `azd up` preprovision lifecycle, including the dedicated Copilot Studio OAuth client, delegated permissions, callback URI support, idempotent secret reuse, azd environment synchronization, documentation, and focused tests. (2026-08-10)
- [x] Changed the MCP Inspector helper from a read-only preset STDIO session to a writable session for local or deployed Streamable HTTP `/mcp` connections, and updated the quick-start guidance. (2026-08-10)
- [x] Redeployed the Streamable HTTP code changes to the existing Azure Container App with `azd deploy mcp` (no Bicep provisioning) and verified the new revision is Running/Healthy with an HTTP 200 MCP initialization response. (2026-08-10)
- [x] Added MCP Streamable HTTP support for Copilot Studio, changed the Azure container entrypoint to serve `/mcp`, updated deployment/testing guidance, and verified a local MCP initialization handshake returns HTTP 200 with a session. (2026-08-08)
