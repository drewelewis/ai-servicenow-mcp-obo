...
4. emit-env: upsert the remaining SERVICENOW_SN_JWT_* values into ``.env``.
...
def _emit_env(args: argparse.Namespace) -> int:
    ...
    values = {
        "SERVICENOW_SN_JWT_TENANT_ID": tenant_id,
        "SERVICENOW_SN_JWT_UPSTREAM_CLIENT_ID": upstream_client_id,
        "SERVICENOW_SN_JWT_CLIENT_ID": args.client_id,
        "SERVICENOW_SN_JWT_PRIVATE_KEY_PATH": args.private_key_path,
        "SERVICENOW_SN_JWT_TOKEN_ENDPOINT": token_endpoint,
        "SERVICENOW_SN_JWT_USER_CLAIM_SOURCE": args.user_claim_source,
    }
    if args.client_secret:
        values["SERVICENOW_SN_JWT_CLIENT_SECRET"] = args.client_secret
    if args.scope:
        values["SERVICENOW_SN_JWT_SCOPE"] = args.scope
    if args.kid:
        values["SERVICENOW_SN_JWT_KID"] = args.kid
    ...
    if missing and lines and lines[-1].strip():
        lines.append("")
    if missing:
        lines.append("# ServiceNow JWT settings (generated by bootstrap_servicenow_jwt.py)")
        lines.extend(f"{key}={values[key]}" for key in missing)

    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Updated {len(values)} ServiceNow JWT settings in {output_path}.")
    return 0
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    emit_env = subparsers.add_parser("emit-env", help="Upsert remaining SERVICENOW_SN_JWT_* values into .env")
    ...
    emit_env.add_argument("--output-file", default=str(REPO_ROOT / ".env"), help="Environment file to update in place")
