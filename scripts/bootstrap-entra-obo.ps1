...
param(
    ...
    [string]$FoundryClientAppName = "servicenow-mcp-foundry-client",
    [Alias("FoundryRedirectUri")][string[]]$FoundryRedirectUris,
    [int]$SecretYears = 1,
...
function Set-AzdEnvironmentValues {
    ...
    foreach ($entry in $Values.GetEnumerator()) {
        $nativeValue = ([string]$entry.Value).Replace('"', '\"')
        azd env set $entry.Key $nativeValue | Out-Null
        if ($LASTEXITCODE -ne 0) {
...
function Update-LocalEnvFile {
    ...
    if ($missingKeys.Count -gt 0) {
        ...
        $lines += "# Generated OAuth settings (managed by azd preprovision)"
...
function Grant-DelegatedPermission {
    ...
    if ($null -ne $resourceAccess -and $resourceAccess.PSObject.Properties["resourceAccess"]) {
...
function Set-WebRedirectUris {
    ...
    Write-Host "Reconciling web redirect URIs for $Label."
    az ad app update --id $AppObjectId --web-redirect-uris $normalizedRedirectUris --output none
    if ($LASTEXITCODE -ne 0) {
        throw "Could not reconcile web redirect URIs for $Label."
    }
}
...
$foundryClientApp = Ensure-App -DisplayName $FoundryClientAppName
...
Ensure-ServicePrincipal -AppId $foundryClientApp.appId
...
Grant-DelegatedPermission -ClientAppId $foundryClientApp.appId -ResourceAppId $brokerApp.appId -ScopeId $brokerScopeId -ScopeName $BrokerScopeName -ClientLabel "Foundry client app"
...
if (($null -eq $FoundryRedirectUris -or $FoundryRedirectUris.Count -eq 0) -and $azdValues.ContainsKey("FOUNDRY_REDIRECT_URIS_JSON")) {
    $FoundryRedirectUris = @(ConvertFrom-Json -InputObject $azdValues["FOUNDRY_REDIRECT_URIS_JSON"])
}

if ($null -ne $FoundryRedirectUris -and $FoundryRedirectUris.Count -gt 0) {
    Set-WebRedirectUris -AppObjectId $foundryClientApp.id -RedirectUris $FoundryRedirectUris -Label "Foundry client app"
} else {
    Write-Warning "Foundry callback URI is not available. Save the Foundry credential provider, set FOUNDRY_REDIRECT_URIS_JSON, then rerun azd up."
}
...
$existingFoundrySecret = if ($azdValues.ContainsKey("FOUNDRY_CLIENT_SECRET")) { $azdValues["FOUNDRY_CLIENT_SECRET"] } else { $null }
...
$foundryClientSecret = Get-OrCreateClientSecret -AppId $foundryClientApp.appId -DisplayName "servicenow-mcp-foundry" -Years $SecretYears -ExistingSecret $existingFoundrySecret -Rotate:$RotateSecrets
...
$generatedValues = @{
    ...
    "FOUNDRY_CLIENT_ID" = $foundryClientApp.appId
    "FOUNDRY_CLIENT_SECRET" = $foundryClientSecret
    "FOUNDRY_SCOPE" = $copilotStudioScopeValue
    "FOUNDRY_AUTHORIZATION_URL" = $authorizationEndpoint
    "FOUNDRY_TOKEN_URL" = $tokenEndpoint
    "FOUNDRY_REFRESH_URL" = $tokenEndpoint
}
...
if ($null -ne $FoundryRedirectUris -and $FoundryRedirectUris.Count -gt 0) {
    $generatedValues["FOUNDRY_REDIRECT_URI"] = $FoundryRedirectUris[-1]
    $generatedValues["FOUNDRY_REDIRECT_URIS_JSON"] = ConvertTo-Json -InputObject @($FoundryRedirectUris) -Compress
}
